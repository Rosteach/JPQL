...
public class MerchantDaoImpl implements MerchantDao {
    ...
    public List<Result> getTotalReport(){
        String txt = "SELECT new com.bionic.edu.Result(m.id, m.name, m.minSum, SUM(p.sumPayed), m.needToSend)";
        txt += "FROM Payment p, Merchant m WHERE m.id = p.merchantId GROUP BY m.id, m.name, m.minSum, m.needToSend"; 
